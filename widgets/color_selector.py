...
class ColorSelector(ttk.Frame):
    ...
    def update_color_value(self, color, *_):
        ...
        self.update_colored_button()

        self.update_in_progress = False

    def update_colored_button(self):
        ...
        if sum_of_color_values > self.foreground_threshold:
            self.colored_button.configure(foreground='black', activeforeground='black')
        else:
            self.colored_button.configure(foreground='white', activeforeground='white')
        self.colored_button.configure(background=color_code, activebackground=color_code, text=color_code)
    ...
    def keys_pressed(top_widget, entry_text_var, event:tk.Event):
        min_value = 0
        max_value = 255
